...
class PubChemScraper(BaseScraper):
    ...
    def __init__(self):
        ...
        self.search_url = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/{}/cids/JSON"
        self.properties_url = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/{}/property/{}/JSON"
        self.record_url = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/{}/JSON"
    ...
    def extract_chemical_data(self, identifier: Union[str, Dict[str, str]]) -> Dict[str, any]:
        ...
        try:
            ...
            chemical_data = {
                ...
                "precautionary_statements": ghs_data.get("precautionary_statements", ""),
                ...
                if "TOCHeading" in section and section["TOCHeading"] == "GHS Classification":
                    ...
                    for subsection in section["Section"]:
                        ...
                        if heading == "GHS Hazard Statements":
                            ...
                            if "Information" in subsection and subsection["Information"]:
        ...
        try:
            ...
            def extract_property(section_name, property_name):
                for section in data["Record"]["Section"]:
                    if "TOCHeading" in section and section["TOCHeading"] == section_name:
                        if "Section" not in section:
                            continue
                            
                        for subsection in section["Section"]:
                            if "TOCHeading" in subsection and subsection["TOCHeading"] == property_name:
                                if "Information" in subsection and subsection["Information"]:
                                    info = subsection["Information"][0]
                                    if "Value" in info and "StringWithMarkup" in info["Value"]:
                                        markup = info["Value"]["StringWithMarkup"][0]
            ...
            result["physical_state"] = extract_property("Experimental Properties", "Physical Description")
            ...
            result["melting_point"] = extract_property("Experimental Properties", "Melting Point")
            result["boiling_point"] = extract_property("Experimental Properties", "Boiling Point")
            result["flash_point"] = extract_property("Safety and Hazards", "Flash Point")
            result["solubility"] = extract_property("Experimental Properties", "Solubility")
            result["vapor_pressure"] = extract_property("Experimental Properties", "Vapor Pressure")
